...
1. **克隆项目**
```bash
git clone https://https://github.com/laihya/music-sync-web.git
cd music-sync-web
配置环境变量
...
FAILED_PATH=/your/failed/path
DATA_PATH=/your/data/path

# AcoustID API Key（可选，留空则使用官方测试Key）
ACOUSTID_API_KEY=

# Discogs API Key（可选，留空则不启用）
DISCOGS_API_KEY=
...
提取指纹：fpcalc -json <file>，输出音频的 Chromaprint 指纹和时长

查找 AcoustID：携带指纹和 API Key 请求 api.acoustid.org/v2/lookup

获取 MusicBrainz 元数据：使用返回的录音 ID 调用 musicbrainzngs.get_recording_by_id
...
❓ 常见问题
Q：下载目录是只读的，为什么文件还能移动？
A：源文件始终保留，处理时先复制到容器内临时目录，所有修改都在副本上进行，最后用 shutil.move 将最终文件移至媒体库，源文件完好。

Q：为什么国内源默认关闭？
A：出于版权合规考虑，本项目不内置任何国内音乐平台的 API 地址。用户需自行获取合法的接口地址，并在 Web 界面中配置。

Q：如何添加国内源？
A：在 Web 界面“配置”中，先开启对应源的卡片，然后在“换源配置”中填入有效的 API URL，保存配置后即刻生效。

Q：声纹匹配需要额外配置吗？
A：不需要。声纹匹配默认使用官方测试 Key I5CvINoX9AI，免费可用。如需提高稳定性，可前往 AcoustID 官网 免费注册一个 Key，并在 .env 中配置 ACOUSTID_API_KEY=你的Key。
