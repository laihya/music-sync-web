...
- **软件依赖**：
  - Docker 20.10+
  - Docker Compose v2+
- **宿主机目录**：需要四个**真实存在且可读写**的目录（下载目录只需**只读**权限）
...
├── setup.sh              # 一键部署脚本
├── README.md             # 项目总览
├── Configuration and Build Guide.md # 部署与配置指南
└── app/                  # 应用源码
    ├── main.py
    ├── config.py
    ├── database.py
...
3. **保存配置**：点击「保存配置」按钮，看到“配置已保存”弹窗即生效。
4. **手动触发扫描**：点击「扫描下载目录中已存在的音乐文件」按钮。

---

## 六、获取 API Key（可选）

- **AcoustID**：访问 [https://acoustid.org/login](https://acoustid.org/login) → 用 GitHub / Google 登录 → Applications → Register Application → 获取 API Key → 写入 `.env` 中的 `ACOUSTID_API_KEY`。
- **Discogs**：访问 [https://www.discogs.com/settings/developers](https://www.discogs.com/settings/developers) → 生成 Personal Access Token → 写入 `.env` 中的 `DISCOGS_API_KEY`。
...
<details>
<summary>❌ AcoustID 声纹匹配无结果</summary>
